...
// source: https://github.com/qbzzt/20220915-all-you-can-cache/blob/main/src/Cache.sol
...
contract Cache {
    ...
    mapping(uint256 => uint256) public val2key;

    // Location n has the value for key n+1, because we need to preserve
    // zero as "not in the cache".
    uint256[] public key2val;

    function cacheRead(uint256 _key) public view returns (uint256) {
    ...
    function cacheWrite(uint256 _value) public returns (uint256) {
        ...
        require(
            key2val.length + 1 < 0x0DFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF,
            "cache overflow"
        );
    ...
    function _calldataVal(uint256 startByte, uint256 length)
        private
        pure
        returns (uint256)
    {
        ...
        require(length < 0x21, "_calldataVal length limit is 32 bytes");
        require(
    ...
    function _readParam(uint256 _fromByte)
        internal
        returns (uint256 _nextByte, uint256 _parameterValue)
    {
        ...
        if (_firstByte == uint8(INTO_CACHE)) {
            uint256 _param = _calldataVal(_fromByte + 1, 32);
            cacheWrite(_param);
